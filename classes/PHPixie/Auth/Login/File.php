...
class File extends Provider {
	...
	public function login($login, $password, $persist_login = false) {
		$user = $this->service->user_file($login);
		if($user->exists){
	 ...
	 *
	 * @param \stdClass $user User model to update
	 * @return string Generated token
	 */
	public function regenerate_login_token($user) {
		$token = $this->random_string().':'.time();
		$user->token = $token;
		$this->service->user_file($user->login,$user);
		return $token;
	}
	 ...
	 *
	 * @param \stdClass $user User model to check
	 ...
	 * Login user by token
	 *
	 * @param string $user_token Token to verify the user against
	 *
	 * @return bool If the user exists.
	 */
